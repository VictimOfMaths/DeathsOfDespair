...
temp <- tempfile()
source <- "https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/alcoholspecificdeathsintheuk/current/alcoholspecificdeaths2023.xlsx"
temp <- curl_download(url=source, destfile=temp, quiet=FALSE, mode="wb")

#Country-specific figures
UKraw <- read_excel(temp, sheet="Table_1", range="A6:O28", col_names=FALSE) %>% 
...
Engraw <- read_excel(temp, sheet="Table_2", range="A6:O28", col_names=FALSE) %>% 
  set_names("code", "Country", "Year", "Deaths_Total", "Rate_Total", "Lower_Total", 
            "Upper_Total", "Deaths_Male", "Rate_Male", "Lower_Male", "Upper_Male",
            "Deaths_Female", "Rate_Female", "Lower_Female", "Upper_Female")

Walraw <- read_excel(temp, sheet="Table_3", range="A6:O28", col_names=FALSE) %>% 
  set_names("code", "Country", "Year", "Deaths_Total", "Rate_Total", "Lower_Total", 
            "Upper_Total", "Deaths_Male", "Rate_Male", "Lower_Male", "Upper_Male",
            "Deaths_Female", "Rate_Female", "Lower_Female", "Upper_Female")  

Scoraw <- read_excel(temp, sheet="Table_4", range="A6:O28", col_names=FALSE) %>% 
  set_names("code", "Country", "Year", "Deaths_Total", "Rate_Total", "Lower_Total", 
            "Upper_Total", "Deaths_Male", "Rate_Male", "Lower_Male", "Upper_Male",
            "Deaths_Female", "Rate_Female", "Lower_Female", "Upper_Female")

NIraw <- read_excel(temp, sheet="Table_5", range="A6:O28", col_names=FALSE) %>% 
  set_names("code", "Country", "Year", "Deaths_Total", "Rate_Total", "Lower_Total", 
            "Upper_Total", "Deaths_Male", "Rate_Male", "Lower_Male", "Upper_Male",
            "Deaths_Female", "Rate_Female", "Lower_Female", "Upper_Female")
...
ggplot(data %>% filter(Country=="United Kingdom" & Sex=="Total"), 
       aes(x=Year, y=Deaths))+
...
ggplot(data %>% filter(Country!="United Kingdom" & Sex=="Total") %>% 
  ...
  scale_fill_manual(name="", values=c("#017a4a", "#FFCE4E", "#3d98d3", "#ff363c"),
                    guide = guide_legend(reverse = TRUE))+
  theme_custom()+
  labs(title="Alcohol-specific deaths across the UK",
       subtitle="Age-standardised mortality rates from conditions caused only by alcohol.\nShaded areas represent 95% Confidence Intervals.",
       caption="Data from ONS, OHID, NRS and NISRA | Plot by @VictimOfMaths")

dev.off()

agg_png("Outputs/ASDEngScot.png", units="in", width=16/1.5, height=9/1.5, res=800)
ggplot(data %>% filter(Country %in% c("England", "Scotland") & Sex=="Total"),
       aes(x=Year, y=Rate, ymin=Lower, ymax=Upper))+
  geom_hline(yintercept=0, colour="grey30")+
  geom_ribbon(aes(fill=Country), alpha=0.1)+
  geom_line(aes(colour=Country))+
  scale_x_continuous(name="")+
  scale_y_continuous(name="Age-standardised deaths per 100,000")+
  scale_colour_manual(name="", values=c("red", "darkblue"),
                      guide = guide_legend(reverse = TRUE))+
...
ggplot(data %>% filter(Country=="England" & Sex=="Total"), 
       aes(x=Year, y=Deaths))+
  geom_hline(yintercept=0, colour="grey30")+
  geom_line(colour="tomato")+
  scale_x_continuous(name="")+
...
#Plot showing relative impacts of MUP in Scotland vs England
agg_png("Outputs/ASDScotEngRatio.png", units="in", width=9, height=6, res=800)
data %>% filter(Country %in% c("England", "Scotland") & Sex=="Total") %>% 
  group_by(Year) %>% 
  summarise(Ratio=Rate[Country=="Scotland"]/Rate[Country=="England"], .groups="drop") %>% 
  ggplot(aes(x=Year, y=Ratio))+
  geom_hline(yintercept=1, colour="grey20")+
  geom_vline(xintercept=2018.1, linetype=2, colour="grey70")+
  geom_line(colour="hotpink")+
  scale_x_continuous(name="")+
  scale_y_continuous(name="Ratio of alcohol-specific deaths in Scotland to England\n(log scale)", trans="log",
                     breaks=c(1,1.5, 2, 3), labels=c("Parity", "50% higher", "Double", "Triple"))+
  theme_custom()+
  annotate("text", x=2018.4, y=2.5, label="MUP introduced", colour="grey50", family="Lato", angle=90)+
  labs(title="The gap in alcohol deaths between England and Scotland continues to narrow",
       subtitle="Ratio of deaths from conditions only caused by alcohol in Scotland and England\n",
...
changes <- data %>% 
  ...
  summarise(RateChange=(Rate[Year==2023]-Rate[Year==2019])/Rate[Year==2019], .groups="drop")
...
ggplot(changes %>% filter(Country!="United Kingdom" & Sex!="Total") %>% 
  ...
  scale_x_continuous(name="Change 2019-2023", labels=label_percent(accuracy=1),
                     limits=c(-0.1, 0.6))+
  ...
  labs(title="Alcohol-specific deaths have risen fastest in men, except in England",
       subtitle="Change in age-standardised mortality rates from conditions caused only by alcohol between 2019 and 2023 by sex\n ",
       caption="Data from ONS | Plot by @VictimOfMaths")

dev.off()

agg_png("Outputs/ASDxSex.png", units="in", width=11, height=5, res=800)
bind_rows(Engraw, Walraw, Scoraw, NIraw) %>% 
  group_by(Year) %>% 
  ...
  theme_custom()+
  labs(title="Across the UK, rising alcohol deaths are driven by men",
       subtitle="Number of deaths in the UK from conditions caused only by alcohol",
       caption="Data from ONS | Plot by @VictimOfMaths")
...
#By region
regdata <- read_excel(temp, sheet="Table_6", range="A6:H626", col_names=FALSE) %>% 
  set_names("Year", "Sex", "Code", "Region", "Deaths", "Rate", "Lower", "Upper") %>% 
  group_by(Region, Sex) %>% 
  mutate(RateChange=(Rate[Year==2023]-Rate[Year==2019])/Rate[Year==2019]) %>% 
  ungroup()

agg_png("Outputs/ASDxRegion.png", units="in", width=8, height=6, res=800)
ggplot(regdata %>% filter(Sex=="Persons"),
       aes(x=Year, y=Rate))+
  geom_rect(ymin=0, ymax=30, xmin=2019.2, xmax=2023.5, colour="grey90", fill="grey90")+
  geom_hline(yintercept=0, colour="grey30")+
  geom_ribbon(aes(ymin=Lower, ymax=Upper), alpha=0.2, fill="tomato")+
  geom_line(colour="tomato")+
  geom_text(data=. %>% filter(Year==2023), aes(label=paste0("+", round(RateChange*100, 0), "%")), 
  ...
  labs(title="Alcohol deaths have risen sharply across all regions of England",
       subtitle="Age-standardised mortality rates from conditions that are only caused by alcohol by English region.\nGrey shaded areas represent the pandemic period and the percentage change 2019-2023.",
       caption="Data from ONS | Plot by @VictimOfMaths")

dev.off()

#By condition
conddata <- read_excel(temp, sheet="Table_9", range="A4:AA1039") %>% 
...
#By age
agedata <- read_excel(temp, sheet="Table_7", range="A6:CZ74", col_names=FALSE) %>% 
  ...
  pivot_longer(cols=c(5:104), names_to=c("Age", "Metric"), names_sep="_", values_to="Value") %>% 
...
ggplot(agedata %>% filter(Sex=="Persons" & Metric=="Rate") %>% arrange(Year),
  ...
  labs(title="Alcohol-specific deaths have risen sharply in older ages",
       subtitle="Age-specific trends in deaths from conditions caused only by alcohol between 2001 and 2023 in the UK",
       caption="Data from ONS | Plot by @VictimOfMaths")

dev.off()

agedata %>% filter(Sex=="Persons" & Metric=="Rate" & Year>=2022) %>% 
  ggplot(aes(x=Year, y=Value, colour=Age))+
  geom_line()+
  theme_custom()

temp <- agedata %>% filter(Age=="50-54" & Year >=2022)

#Calculate age-standardised rates
ASdata <- agedata %>% 
  mutate(weight=case_when(
    Age=="<1" ~ 1000, Age=="1-4" ~ 4000, Age=="5-9" ~ 5500, Age=="10-14" ~ 5500,
    Age=="15-19" ~ 5500, Age=="20-24" ~ 6000, Age=="25-29" ~ 6000, Age=="30-34" ~ 6500,
    Age=="35-39" ~ 7000, Age=="40-44" ~ 7000, Age=="45-49" ~ 7000, Age=="50-54" ~ 7000,
    Age=="55-59" ~ 6500, Age=="60-64" ~ 6000, Age=="65-69" ~ 5500, Age=="70-74" ~ 5000,
    Age=="75-79" ~ 4000, Age=="80-84" ~ 2500, Age=="85-89" ~ 1500, Age=="90+" ~ 1000)) %>% 
  filter(Metric=="Rate") %>% 
  mutate(Value=if_else(is.na(Value), 0, Value)) %>% 
  group_by(Sex, Country, Year) %>% 
  summarise(Rate=weighted.mean(Value, weight), .groups="drop")
