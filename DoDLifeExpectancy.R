...
#https://www.ons.gov.uk/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/the21stcenturymortalityfilesdeathsdataset/current
ewfile <- tempfile()
ewurl <- "https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/the21stcenturymortalityfilesdeathsdataset/current/21stcmortality.xlsx"
ewfile <- curl_download(url=ewurl, destfile=ewfile, quiet=FALSE, mode="wb")

ewdata.01 <- read_excel(ewfile, sheet="1", range="A5:E21265")
ewdata.02 <- read_excel(ewfile, sheet="2", range="A5:E20880")
ewdata.03 <- read_excel(ewfile, sheet="3", range="A5:E21251")
ewdata.04 <- read_excel(ewfile, sheet="4", range="A5:E20959")
ewdata.05 <- read_excel(ewfile, sheet="5", range="A5:E20928")
ewdata.06 <- read_excel(ewfile, sheet="6", range="A5:E20866")
ewdata.07 <- read_excel(ewfile, sheet="7", range="A5:E20657")
ewdata.08 <- read_excel(ewfile, sheet="8", range="A5:E20660")
ewdata.09 <- read_excel(ewfile, sheet="9", range="A5:E20792")
ewdata.10 <- read_excel(ewfile, sheet="10", range="A5:E20784")
ewdata.11 <- read_excel(ewfile, sheet="11", range="A5:E20380")
ewdata.12 <- read_excel(ewfile, sheet="12", range="A5:E20211")
ewdata.13 <- read_excel(ewfile, sheet="13", range="A5:E20439")
ewdata.14 <- read_excel(ewfile, sheet="14", range="A5:E20426")
ewdata.15 <- read_excel(ewfile, sheet="15", range="A5:E20198")
ewdata.16 <- read_excel(ewfile, sheet="16", range="A5:E20280")
ewdata.17 <- read_excel(ewfile, sheet="17", range="A5:E20193")
ewdata.18 <- read_excel(ewfile, sheet="18", range="A5:E20481")
ewdata.19 <- read_excel(ewfile, sheet="19", range="A5:E20305")
ewdata.20 <- read_excel(ewfile, sheet="20", range="A5:E19032")
ewdata.21 <- read_excel(ewfile, sheet="21", range="A5:E19157")
...
ewdata <- bind_rows(ewdata.01, ewdata.02, ewdata.03, ewdata.04, ewdata.05, ewdata.06, ewdata.07,
                    ...
                    ewdata.15, ewdata.16, ewdata.17, ewdata.18, ewdata.19, ewdata.20, ewdata.21) %>% 
  #Allocate causes to code groups
  mutate(code1=substr(`ICD-10`, 1, 1), code2=as.numeric(substr(`ICD-10`,2,3)), 
         code3=as.numeric(substr(`ICD-10`,4,4)),
         Cause=case_when(
  ...
  summarise(Dx=sum(`Number of deaths`)) %>% 
  ungroup()
...
ewpop.grouped <- ewpop %>% 
  ...
  summarise(across(`2001`:`2020`, sum, na.rm=TRUE)) %>% 
  ungroup()
...
#Age groups are 90+ in 2016-2020, but 85+ before then
scotdata <- bind_rows(scotdata.2006, scotdata.2007, scotdata.2008, scotdata.2009, scotdata.2010, 
  ...
  #Even more faff because the 2021 data is also horribly formatted, but differently so
scotdata <- scotdata.2021 %>% 
  #The first 26 rows have different formatting to everything else because of MYSTERY REASONS
  mutate(`...2`=if_else(row_number()<27 & is.na(`...2`), lag(`...2`,1), `...2`)) %>% 
  filter(!is.na(`...2`)) %>% 
  rename(ICD10=`...2`, Sex=`...3`) %>% 
  mutate(ICD10=substr(ICD10, 1, 3)) %>% 
  select(-c(`...1`, `...4`)) %>% 
  gather(Age, Dx, c(3:22)) %>% 
  mutate(Dx=as.numeric(if_else(Dx %in% c("-", ".", NA), "0", Dx)),
         Age=case_when(Age=="...5" ~ "0", Age=="...6" ~ "1-4",Age=="...7" ~ "5-9",
                       Age=="...8" ~ "10-14", Age=="...9" ~ "15-19", Age=="...10" ~ "20-24",
                       Age=="...11" ~ "25-29", Age=="...12" ~ "30-34", Age=="...13" ~ "35-39",
...
scotdata.wide <- scotdata %>% 
  spread(Year, Dx) %>%
  mutate(across(c(`2001`:`2021`), ~replace_na(.x, 0))) %>% 
  group_by(Age, Sex) %>% 
  summarise(across(c(`2001`:`2021`), sum)) %>% 
  ungroup() %>% 
  mutate(Cause="Total") %>% 
...
nidata <- bind_rows(nidata.2001, nidata.2002, nidata.2003, nidata.2004, nidata.2005, nidata.2006, 
  ...
  bind_rows(bind_rows(nidata.2012, nidata.2013, nidata.2014, nidata.2015, nidata.2016,
              ...
              mutate(Age=case_when(
                ...
                Dx=as.numeric(Dx), Dx=if_else(is.na(Dx), 0, Dx)) %>% 
              set_names(c("ICD10", "Cause", "Sex", "Year", "Age", "Dx"))) %>% 
  bind_rows(nidata.2020 %>% gather(Age, Dx, c(5:24)) %>% 
              select(-`...2`) %>% 
              mutate(Age=case_when(
                Age=="...3" ~ "0", Age=="...4" ~ "1-4", Age=="...5" ~ "5-9", Age=="...6" ~ "10-14",
                Age=="...7" ~ "15-19", Age=="...8" ~ "20-24", Age=="...9" ~ "25-29", Age=="...10" ~ "30-34",
                Age=="...11" ~ "35-39", Age=="...12" ~ "40-44", Age=="...13" ~ "45-49", Age=="...14" ~ "50-54",
                Age=="...15" ~ "55-59", Age=="...16" ~ "60-64", Age=="...17" ~ "65-69", Age=="...18" ~ "70-74",
                Age=="...19" ~ "75-79", Age=="...20" ~ "80-84", Age=="...21" ~ "85-89", Age=="...22" ~ "90+"),
                ICD10=gsub("\\)", "", ICD10))) %>% 
...
niallcause <- bind_rows(niallcause.2001, niallcause.2002, niallcause.2003, niallcause.2004, 
  ...
  bind_rows(bind_rows(niallcause.2012, niallcause.2013, niallcause.2014, niallcause.2015,
              ...
              mutate(Age=case_when(
                ...
                Cause="Total") %>% 
              set_names(c("Sex", "Year", "Age", "Dx", "Cause"))) %>% 
  bind_rows(niallcause.2020 %>% select(-c(ICD10, Cause, `...2`)) %>% 
              gather(Age, Dx, c(2:21)) %>% 
              mutate(Age=case_when(
                Age=="...3" ~ "0", Age=="...4" ~ "1-4", Age=="...5" ~ "5-9", Age=="...6" ~ "10-14",
                Age=="...7" ~ "15-19", Age=="...8" ~ "20-24", Age=="...9" ~ "25-29", Age=="...10" ~ "30-34",
...
nidata.wide <- nidata %>% 
  ...
  spread(Year, Dx) %>% 
  mutate(across(c(`2001`:`2020`), ~replace_na(.x, 0))) %>% 
  #Initiate start of age groups
  mutate(agestart=case_when(
    Age=="0" ~ 0,
    Age=="1-4" ~ 1, Age=="5-9" ~ 5, Age=="10-14" ~ 10, Age=="15-19" ~ 15,
...
UKdata <- ewdata.wide %>% 
  ...
  merge(ewpop.grouped %>% gather(Year, pop, c(3:22))) %>% 
  mutate(mx=Dx*100000/pop) %>% 
  bind_rows(scotdata.wide %>% 
              gather(Year, Dx, c(5:25)) %>% 
              select(Cause, agestart, Sex, Year, Dx) %>% 
              mutate(Country="Scotland") %>%
              merge(scotpop.grouped %>% gather(Year, pop, c(3:22))) %>% 
              mutate(mx=Dx*100000/pop)) %>% 
  bind_rows(nidata.wide %>% 
              gather(Year, Dx, c(5:24)) %>% 
              select(Cause, agestart, Sex, Year, Dx) %>% 
              mutate(Country="Northern Ireland") %>%
              merge(nipop.grouped %>% gather(Year, pop, c(3:22))) %>% 
              mutate(mx=Dx*100000/pop)) 

######################################################################################
#Repeat for US by state and overall
#Data all downloaded from CDC wonder using ICD-10 definitions as above

#Read in data
USrawAlcMale <- read.csv("CDC Data/CDCWonderAlcMalexState.txt", sep="\t") %>% 
  mutate(Cause="Alcohol")

USrawAlcFemale <- read.csv("CDC Data/CDCWonderAlcFemalexState.txt", sep="\t") %>% 
  mutate(Cause="Alcohol") %>% 
  select(-Gender.Code)

USrawDrgMale <- read.csv("CDC Data/CDCWonderDrgMalexState.txt", sep="\t") %>% 
  mutate(Cause="Drugs", Population=as.character(Population))

USrawDrgFemale <- read.csv("CDC Data/CDCWonderDrgFemalexState.txt", sep="\t") %>% 
  mutate(Cause="Drugs")%>% 
  select(-Gender.Code)

USrawScdMale <- read.csv("CDC Data/CDCWonderScdMalexState.txt", sep="\t") %>% 
  mutate(Cause="Suicide")

USrawScdFemale <- read.csv("CDC Data/CDCWonderScdFemalexState.txt", sep="\t") %>% 
  mutate(Cause="Suicide") %>% 
  select(-Gender.Code)

USrawAllCauseMale <- read.csv("CDC Data/CDCWonderAllCauseMalexState.txt", sep="\t") %>% 
  mutate(Cause="Total")

USrawAllCauseFemale <- read.csv("CDC Data/CDCWonderAllCauseFemalexState.txt", sep="\t") %>% 
  mutate(Cause="Total") %>% 
  select(-Gender.Code)

USrawCOVID <- read.csv("CDC Data/CDCWonderCOVIDxState.txt", sep="\t") %>% 
  mutate(Cause="COVID")

#Stick together and tidy up
USrawdata <- bind_rows(USrawAlcMale, USrawAlcFemale, USrawDrgMale, USrawDrgFemale,
                       USrawScdMale, USrawScdFemale, USrawAllCauseMale, USrawAllCauseFemale, USrawCOVID) %>%
  filter(State!="") %>% 
  select(State, State.Code, Five.Year.Age.Groups.Code, Year, Gender, Deaths, Population, Cause) %>% 
  mutate(Population=as.numeric(Population)) %>% 
  rename(Age=Five.Year.Age.Groups.Code)

#Get population data for ages 85+ from US census bureau
#https://www2.census.gov/programs-surveys/popest/datasets/
USpop1 <- tempfile()
USpopurl1 <- "https://www2.census.gov/programs-surveys/popest/datasets/2000-2009/state/asrh/sc-est2009-alldata5-all.csv"
USpop1 <- curl_download(url=USpopurl1, destfile=USpop1, quiet=FALSE, mode="wb")

USpop0009 <- read_csv(USpop1) %>% 
  filter(ORIGIN==0) %>% 
  select(STATE, SEX, AGE, starts_with("POPESTIMATE")) %>% 
  group_by(STATE, SEX, AGE) %>% 
  summarise(across(starts_with("POPESTIMATE"), sum), .groups="drop")
            
USpop2 <- tempfile()
USpopurl2 <- "https://www2.census.gov/programs-surveys/popest/datasets/2010-2020/state/asrh/SC-EST2020-ALLDATA5.csv"
USpop2 <- curl_download(url=USpopurl2, destfile=USpop2, quiet=FALSE, mode="wb")

USpop1020 <- read_csv(USpop2) %>%
  filter(ORIGIN==0) %>% 
  select(STATE, SEX, AGE, starts_with("POPESTIMATE")) %>% 
  group_by(STATE, SEX, AGE) %>% 
  summarise(across(starts_with("POPESTIMATE"), sum), .groups="drop")

USpop <- merge(USpop0009, USpop1020) %>% 
  mutate(Age=case_when(
    AGE==0 ~ "1", AGE<5 ~ "1-4", AGE<10 ~ "5-9",  AGE<15 ~ "10-14", AGE<20 ~ "15-19", AGE<25 ~ "20-24",
    AGE<30 ~ "25-29", AGE<35 ~ "30-34", AGE<40 ~ "35-39", AGE<45 ~ "40-44", AGE<50 ~ "45-49",
    AGE<55 ~ "50-54", AGE<60 ~ "55-59", AGE<65 ~ "60-64", AGE<70 ~ "65-69", AGE<75 ~ "70-74",
    AGE<80 ~ "75-79", AGE<85 ~ "80-84", TRUE ~ "85+")) %>% 
  group_by(Age, STATE, SEX) %>% 
  summarise(across(starts_with("POPESTIMATE"), sum), .groups="drop") %>% 
  gather(Year, CensusPop, c(4:ncol(.))) %>% 
  mutate(Year=as.numeric(substr(Year, 12, 15)),
         Gender=case_when(
           SEX==1 ~ "Male", SEX==2 ~ "Female", SEX==3 ~ "Both"),
...
USNatPop <- USpop %>% 
  ...
  summarise(CensusPop=sum(CensusPop), .groups="drop")

USdata <- USrawdata %>% 
  #Collapse 85+ age groups to align with population data
  mutate(Age=if_else(Age %in% c("85-89", "90-94", "95-99", "100+"), "85+", Age)) %>% 
  group_by(Age, Year, Gender, Cause, State, State.Code) %>% 
  summarise(Deaths=sum(Deaths), .groups="drop") %>% 
  #Fill in gaps where state/age group combinations have no all-cause deaths (or <10 at least)
  #Use 1 not 0 for censored values as lifetable generating function falls over in some instances
  #with 0s. Impact on results likely to be very marginal.
  spread(Age, Deaths) %>% 
  mutate(across(c(`1`:ncol(.)), ~if_else(is.na(.x) & Cause=="Total", 1, as.numeric(.x)))) %>% 
  gather(Age, Deaths, c(`1`:ncol(.))) %>% 
  #Bring in Census-based population data
  merge(USpop) 

#Add in total rows for the whole of the US
USdata <- USdata %>% 
  group_by(Age, Year, Gender, Cause) %>% 
  summarise(Deaths=sum(Deaths, na.rm=TRUE), .groups="drop") %>% 
  mutate(State="US") %>% 
  merge(USNatPop) %>% 
  bind_rows(USdata) %>% 
  mutate(agestart=case_when(
    Age=="1" ~ 0, Age=="1-4" ~ 1, Age=="5-9" ~ 5, TRUE ~ as.numeric(substr(Age, 1, 2))))

rm(list=setdiff(ls(), c("ewdata.wide", "ewpop", "ewpop.grouped", "scotdata.wide", "scotpop", 
                        "scotpop.grouped", "nidata.wide", "nipop", "nipop.grouped", "password",
                        "username", "font", "theme_custom", "USpop", "USNatPop", "USdata", "UKdata")))

##################
#Experiments with LE (note, you may need the dev version of {rstan} to install this on Windows, because of reasons)
...
#Align US and UK datasets
USdataforLE <- USdata %>% 
  mutate(Sex=if_else(Gender=="Male", 1, 2)) %>%
  rename(Geography=State, pop=CensusPop, Dx=Deaths) %>% 
  select(agestart, Sex, Year, Cause, Geography, Dx, pop) %>% 
  spread(Cause, Dx) %>% 
  mutate(across(c("COVID", "Alcohol", "Drugs", "Suicide", "Total"), ~replace_na(.x, 0)), 
         DOD=Alcohol+Drugs+Suicide, Other=Total-DOD-COVID)

UKdataforLE <- UKdata %>% 
  select(agestart, Sex, Year, Cause, Country, Dx, pop) %>% 
  rename(Geography=Country) %>% 
  spread(Cause, Dx) %>% 
  mutate(COVID=replace_na(COVID, 0), DOD=Alcohol+Drugs+Suicide, Year=as.numeric(Year))

UKUSLEdata <- bind_rows(USdataforLE, UKdataforLE)

LEoutcomes <- data.frame(Geography=character(), Cause=character(), Sex=character(), Year=character(),
                         e0=character())

#Overall LE trend
for(i in c(unique(UKUSLEdata$Geography))){
  for(k in c(1,2)){
    for(l in 2001:2020){
      workingdata <- UKUSLEdata %>% 
        filter(Geography==i & Year==l & Sex==k) %>% 
        select(agestart, Total, pop)
      
      lifetable <- lt_abridged(Deaths=workingdata$Total, Exposures=workingdata$pop, 
                               Age=workingdata$agestart, Sex=if_else(k==1, "m", "f"))
      
      LEoutcomes <- bind_rows(LEoutcomes,
                              c(Geography=i, Cause="Total", Sex=as.integer(k), Year=l, 
                                e0=lifetable$ex[1]))
...
for(i in c(unique(UKUSLEdata$Geography))){
  for(j in c("Alcohol", "Drugs", "Suicide", "DOD")){
    for(k in c(1,2)){
      for(l in 2001:2020){
        workingdata <- UKUSLEdata %>% 
          filter(Geography==i & Year==l & Sex==k) %>% 
          ...
          select(agestart, Total, cause_dx, pop) %>% 
          mutate(rx=if_else(cause_dx==0, 0, cause_dx/Total), mx=Total/pop, 
                 qx=case_when(
        ...
        LEoutcomes <- bind_rows(LEoutcomes,
                                c(Geography=i, Cause=j, Sex=as.integer(k), Year=l, 
                                  e0=lifetable$ex[1]))
      }
    }
  }
...
#Cause-deleted LE trends for COVID (2020 only)
for(i in c(unique(UKUSLEdata$Geography))){
  for(k in c(1,2)){
    workingdata <- UKUSLEdata %>% 
      filter(Geography==i & Year==2020 & Sex==k) %>% 
      select(agestart, Total, COVID, pop)%>% 
      mutate(rx=if_else(COVID==0, 0, COVID/Total), mx=Total/pop, 
             qx=case_when(
               agestart==0 ~ 1-exp(-1*mx),
               agestart==1 ~ 1-exp(-4*mx),
               TRUE ~ 1-exp(-5*mx)), 
    ...
    LEoutcomes <- bind_rows(LEoutcomes,
                            c(Geography=i, Cause="COVID", Sex=as.integer(k), Year=l, 
                              e0=lifetable$ex[1]))
  }
}

Outputs <- LEoutcomes %>% 
  #Stick in overall figures for pre-pandemic years to give consistent COVID timeseries
  #(i.e. no COVID impact on LE before 2020)
  bind_rows(LEoutcomes %>% filter(Year<2020 & Cause=="Total") %>% 
              mutate(Cause="COVID")) %>% 
  #Sort out formatting
  mutate(Sex=if_else(Sex==1, "Male", "Female"),
         ...
         e0=as.double(e0)) %>% 
  spread(Cause, e0) %>% 
  set_names("Country", "Sex", "Year", "Alcohol_e0", "COVID_e0", "DOD_e0", "Drugs_e0", "Suicide_e0", "Total_e0") %>%
  mutate(Alcohol_loss=Total_e0-Alcohol_e0,
         COVID_loss=Total_e0-COVID_e0,
         DOD_loss=Total_e0-DOD_e0,
         Drugs_loss=Total_e0-Drugs_e0,
         Suicide_loss=Total_e0-Suicide_e0) %>% 
  pivot_longer(cols=c(4:8, 10:14), names_to=c("Cause",".value"), names_sep="_") %>% 
  mutate(Level=if_else(Country %in% c("US", "England & Wales", "Scotland", "Northern Ireland"), "Nation", "State"))

agg_tiff("Outputs/LEDecrementsDOD.tiff", units="in", width=9, height=7, res=600)
ggplot(Outputs %>% filter(Cause %in% c("Alcohol", "Drugs", "Suicide") & Level=="Nation"), 
       aes(x=Year, y=loss, colour=Cause))+
  geom_line()+
  geom_hline(yintercept=0, colour="Black")+
  scale_x_continuous(name="")+
  scale_y_continuous(name="Impact on Life Expectancy\n(years)")+
  scale_colour_manual(values=c("#00A1FF", "#E69F00", "#CC5395"))+
  facet_grid(Sex~Country)+
  theme_custom()+
  theme(panel.grid.major.y=element_line(colour="Grey90"))
dev.off()

agg_tiff("Outputs/LEDecrementsDODv2.tiff", units="in", width=9, height=7, res=600)
ggplot(Outputs %>% filter(Cause %in% c("Alcohol", "Drugs", "Suicide") & Level=="Nation"), 
       aes(x=Year, y=loss, colour=Country))+
  geom_line()+
  geom_hline(yintercept=0, colour="Black")+
  scale_x_continuous(name="")+
  scale_y_continuous(name="Impact on Life Expectancy\n(years)")+
  scale_colour_manual(values=c("#FFCE4E", "#3d98d3", "#ff363c", "#7559a2"))+
  facet_grid(Sex~Cause)+
  theme_custom()+
  theme(panel.grid.major.y=element_line(colour="Grey90"))
dev.off()

agg_tiff("Outputs/LEDecrementsDrugsv2.tiff", units="in", width=9, height=7, res=600)
ggplot(Outputs %>% filter(Cause=="Drugs" & Level=="Nation"), 
       aes(x=Year, y=loss, colour=Country))+
  geom_line()+
  geom_hline(yintercept=0, colour="Black")+
  scale_x_continuous(name="")+
  ...
  theme_custom()+
  theme(panel.grid.major.y=element_line(colour="Grey90"))+
  labs(title="The US is not alone in having a drug deaths crisis",
       subtitle="Impact of drug-related deaths on life expectancy at birth in the UK and USA",
       caption="Data from ONS, NRS, NISRA, CDC and US Census Bureau\nPlot by @VictimOfMaths")
dev.off()


ggplot(Outputs %>% filter(Cause %in% c("Alcohol", "Drugs", "Suicide") & Sex=="Male" &
                            Level=="Nation"),
       aes(x=Year))+
  geom_ribbon(aes(ymin=Total_e0, ymax=e0, fill=Cause), alpha=0.3)+
  geom_line(aes(y=Total_e0), colour="Grey40")+
  geom_line(aes(y=e0, colour=Cause))+
...
agg_tiff("Outputs/LEDrugsUSxState.tiff", units="in", width=10, height=7, res=600)
ggplot(Outputs %>% filter(Cause=="Drugs" & Sex=="Male" & Level=="State"),
       aes(x=Year))+
  geom_ribbon(aes(ymin=0, ymax=loss), fill="Tomato2", alpha=0.1)+
  geom_line(aes(y=loss), colour="Tomato2")+
  geom_hline(yintercept=0, colour="Black")+
  geom_point(data=Outputs %>% filter(Cause=="Drugs" & Sex=="Male" & Level=="State" & 
                                       Year==2020),
             aes(x=Year, y=loss), colour="tomato2")+
  scale_x_continuous(name="")+
  scale_y_continuous(name="Impact on life expectancy (years)", breaks=c(0,-1,-2))+
  facet_geo(~Country, grid="us_state_grid2")+
  theme_custom()+
  theme(strip.text=element_blank(), axis.line.x=element_blank(), 
        axis.ticks.x=element_blank(), axis.text.x=element_blank(),
        panel.grid.major.y=element_line(colour="Grey90"),
        plot.tag.position=c(0.06, 0.805), plot.tag=element_text(colour="Grey40", size=rel(0.7)))+
  labs(title="The US Opioid crisis has had a huge impact on male Life Expectancy",
       subtitle="Impact of drug-related deaths on male life expectancy at birth by state 2001-2020\n",
       caption="Data from CDC Wonder/US Census Bureau\nPlot by @VictimOfMaths",
       tag="              2001                      2020")
dev.off()

agg_tiff("Outputs/LEDODUSxState.tiff", units="in", width=10, height=7, res=600)
ggplot(Outputs %>% filter(Sex=="Male" & Level=="State" & Cause %in% c("Alcohol", "Drugs", "Suicide")),
       aes(x=Year, y=loss, colour=Cause))+
  geom_line()+
  geom_hline(yintercept=0, colour="Black")+
  scale_x_continuous(name="")+
...
USStateMap <- ne_states(country="United States of America", returnclass="sf") %>% 
  left_join(Outputs %>% 
              filter(Sex=="Male" & Level=="State" & Year==2020),
            by=c("name"="Country"))

agg_tiff("Outputs/LEDrugsUSxStateMap.tiff", units="in", width=9, height=6, res=600)
ggplot(USStateMap %>% filter(!name %in% c("Hawaii", "Alaska") & Cause=="Drugs"), 
       aes(fill=loss, geometry=geometry))+
  geom_sf(colour=NA)+
  scale_fill_paletteer_c("viridis::rocket", limits=c(NA, 0), 
                         name="Impact on life expectancy (years)")+
  theme_void()+
  ...
  guides(fill = guide_colorbar(barwidth = unit(20, "lines"), 
                                ...
                                title.hjust = 0.5, 
                                title.position = "top"))+
  labs(title="\nThe US Opioid crisis is not happening everywhere",
       subtitle="Impact of drug-related deaths on male life expectancy at birth in 2020\n",
       caption="Data from CDC Wonder/US Census Bureau\nPlot by @VictimOfMaths")
  
dev.off()

agg_tiff("Outputs/LEDODUSxStateMap.tiff", units="in", width=9, height=6, res=600)
ggplot(USStateMap %>% filter(!name %in% c("Hawaii", "Alaska") & Cause=="DOD"), 
       aes(fill=loss, geometry=geometry))+
  geom_sf(colour=NA)+
  scale_fill_paletteer_c("viridis::mako", limits=c(NA, 0), 
                         name="Impact on life expectancy (years)")+
  theme_void()+
  theme(legend.position="top", 
        plot.title.position="plot", plot.caption.position="plot",
  ...
  guides(fill = guide_colorbar(barwidth = unit(20, "lines"), 
                               ...
                               title.hjust = 0.5, 
                               title.position = "top"))+
  labs(title="\nBut 'Deaths of Despair' are having a significant impact across the US",
       subtitle="Impact of deaths due to drugs, alcohol and suicide on male life expectancy at birth in 2020\n",
       caption="Data from CDC Wonder/US Census Bureau\nPlot by @VictimOfMaths")
